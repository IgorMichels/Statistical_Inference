...
\begin{equation*}
    \begin{split}
        ...
        \beta_1 & = \beta_0 + \dfrac{s_n^2}{2} + \dfrac{n\lambda_0\left(\overline{x}_n^2 - \mu_0\right)^2}{2\left(\lambda_0 + n\right)},
    \end{split}
\end{equation*}
...
Exemplificando essa ideia, tomando $\mu_0$ e $\lambda_0 > n$ teremos, após a atualização dos hiperparâmetros, um valor de $\mu_1$ mais próximo de $\mu_0$ que de $\overline{x}_n$. Dessa forma, um $\lambda_0$ maior que $n$ implica que, de certo modo, estamos confiando mais nos conhecimentos prévios sobre o experimento que nas amostras. Podemos notar também que, se $\lambda_0 \gg n$, a diferença entre $\mu_1$ e $\mu_0$ é muito pequena, o que pode nos levar a ideia do parágrafo anterior, ou seja, a de considerar $\lambda_0$ como uma ``métrica de aversão'', assim, quanto maior $\lambda_0$, mais averso estamos a alterações na atualização do parâmetro $\mu_0$, necessitando, para isso, uma maior quantidade de amostras para uma alteração significativa, quando necessária\footnote{Note que podemos ter $\mu_0$ muito próximo a média de $\mu$, então ser averso ou não acaba sendo, praticamente, irrelevante.}.
...
Após termos visto o desenvolvimento das distribuições a posteriori, vamos ilustrar essas ideias na prática.
...
Primeiramente, note que, dados $\lambda_0$, $\mu_0$, $\alpha_0$ e $\beta_0$, necessitamos apenas de $n$, $\overline{x}_n$ e $s_n^2$ para computar os parâmetros da distribuição a posteriori, a qual já vimos ser Normal-Gama. Agora, perceba que Valciclei fez suas estatística baseado em dados de dez pamonhas, ou seja, $n = 10$. Além disso, as estatísticas que Valciclei conseguiu anotar antes de fazer sua visita de emergência ao banheiro foram, justamente, a média amostral $\left(\overline{x}_n\right)$ e a variância amostral $\left(s_n^2\right)$.
